...
function updateSidebarA11y(sidebar, button, isClosed) {
    ...
    sidebar.setAttribute('data-sidebar-state', isClosed ? 'closed' : 'open');
}

function initSidebarToggle() {
    const sidebar = document.querySelector('[data-dashboard-sidebar]');
    const button = document.querySelector('[data-dashboard-sidebar-toggle]');
    ...
    button.addEventListener('click', () => {
        ...
        updateSidebarA11y(sidebar, button, isClosed);
    });
}

function removeValidationState(card) {
    card.querySelectorAll('[data-expense-error]').forEach((error) => {
        error.remove();
    });

    card.querySelectorAll('.is-invalid').forEach((field) => {
...
function resetCardValues(card, defaultDate) {
    card.querySelectorAll('[data-expense-field]').forEach((field) => {
...
function updateCardIndex(card, index) {
    const legend = card.querySelector('[data-expense-legend]');
...
function normalizeAmountInput(input) {
    const maxDigits = Number.parseInt(input.dataset.maxDigits ?? '10', 10);
...
function updateExpenseControls(form) {
    ...
    const cards = [...list.querySelectorAll('[data-expense-card]')];

    cards.forEach((card, index) => {
        updateCardIndex(card, index);

        const removeButton = card.querySelector('[data-remove-expense]');
    ...
    if (countMessage) {
        countMessage.textContent = `${cards.length} expense form${
            cards.length === 1 ? '' : 's'
        } displayed.`;
    }
}
...
function initExpenseForm() {
    ...
    form.querySelectorAll('[data-expense-amount]').forEach((input) => {
    ...
    addButton.addEventListener('click', () => {
        ...
        newCard.querySelector('[data-expense-field]')?.focus();
    });
    ...
    list.addEventListener('click', (event) => {
        ...
        removeButton.closest('[data-expense-card]')?.remove();
        updateExpenseControls(form);
    });

    form.addEventListener('input', (event) => {
        const amountInput = event.target.closest('[data-expense-amount]');
